...
#: Как часто слать кадр целиком. Экран обновляется кусками, и кусок,
#: испорченный на шине, остаётся на стекле, пока там ничего не поменяется:
#: программа считает его нарисованным. Полный кадр это лечит. Раньше — раз
#: в пять минут, и такой кусок мог висеть все пять; кадр целиком стоит
#: около 0,2 с работы шины, раз в минуту это незаметно.
FULL_REFRESH_EVERY = 60.0
...
class Application:
    ...
    def _render(self, screen) -> None:
        ...
        # Поверх калибровки — нет: там кнопка значит «отмена», а крестики
        # под затемнением не разглядеть.
        if self.director.held_ms and self.director.modal is None:
