...
@dataclass
class Env:
    #: Как менялся CO2 за последние часы, от старого к свежему. Нужен для
    #: спарклайна: одно число говорит, сколько сейчас, но не говорит
    #: главного — растёт оно или падает. А решение проветрить принимают
...
@dataclass
class Weather:
    #: Что будет дальше, по частям суток. Пусто — прогноз не приехал.
    ...
    #: Название места, для которого показана погода. Пусто — не задано.
    #: Нужно на экране: без него «+12, пасмурно» не отличить от погоды в
    #: чужом городе, а координаты человеку ничего не говорят.
    place: str = ""
...
class Desk:
    ...
    def restart_sitting(self, now: datetime) -> None:
        """Забыть, сколько человек сидит, и начать счёт заново.

        Нужно, когда системные часы перевели: до сверки по сети плата
        считает, что сейчас момент прошлого выключения, и запомненное в эти
        секунды «сел за стол» после перевода превращается в часы сидения,
        которых не было.
        """
        self.sitting_since = now if self.presence else None
...
class Health:
    ...
    #: Часы сверены по сети.
    #:
    #: У Pi нет часов реального времени. При выключении система запоминает
    #: время, при включении восстанавливает — и до ответа NTP плата уверена,
    #: что сейчас момент прошлого выключения. Сервис стартует через двенадцать
    #: секунд после ядра, то есть заведомо раньше.
    #:
    #: Пока здесь False, времени верить нельзя: записанное в историю уедет
    #: в прошлое, а счётчики времени насчитают часы, которых не было.
