...
class Source(ABC):
    ...
    def healthy(self) -> bool:
        ...
        По умолчанию это просто «последний опрос не упал». Но датчик умеет
        отказывать молча: SCD41 после сбоя питания исправно отвечает по
        шине и отдаёт нули, исключения при этом нет. Наследники, которые
        такое умеют, переопределяют это свойство — иначе экран показывал бы
        позавчерашний CO2 и уверял, что всё хорошо.
        """
        return self.ok
