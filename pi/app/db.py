...
class Database:
    ...
    def stored_bytes(self) -> int:
        """Размер самой базы, без журнала.

        Отдельно от size_bytes, потому что вопросы разные. «Сколько занято
        на карте прямо сейчас» — это вместе с журналом. «На сколько база
        вырастет за год» — только сама база: журнал WAL упирается в свой
        потолок около четырёх мегабайт и дальше переиспользуется, он не
        накапливается. Считать рост вместе с ним значит обещать полтора
        гигабайта в год там, где будет пятьдесят мегабайт.
        """
