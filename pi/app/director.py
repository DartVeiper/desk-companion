...
class Director:
    def __init__(
        ...
        #: Экран поверх всего, который нельзя пролистать, — калибровка тача.
        #: В обычный стек накладок он не кладётся: оттуда его закрыл бы
        #: таймаут, свайп или уход в покой посреди калибровки.
        self.modal: Screen | None = None
        #: Что делать, если в модальном экране нажали кнопку энкодера.
        #: Кнопка — единственный выход, который работает, когда тач как раз
        #: и не откалиброван.
        self.on_modal_cancel = None
    ...
    def handle(self, event: InputEvent, state: State) -> None:
        if self.modal is not None:
            # Вращение и прочее в модальном экране не значат ничего:
            # листать нельзя. Нажатие кнопки — отмена.
            if event.action in (Action.SELECT, Action.HOLD, Action.SETTINGS, Action.BACK):
                if self.on_modal_cancel is not None:
                    self.on_modal_cancel()
            return
