...
ALLOWED = {
    ...
    #: Где мы, для погоды. Здесь, а не только в config.toml, потому что
    #: конфиг приезжает с кодом: любой, кто соберёт себе такой же блок,
    #: получил бы погоду в Белгороде и менять её должен был бы правкой
    #: файла на плате по ssh. Название держим рядом с координатами —
    #: показать на экране «50.6, 36.6» означает не показать ничего.
    "location.lat": float,
    "location.lon": float,
    "location.name": str,
