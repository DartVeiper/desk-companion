...
# Сеть НЕ ждём намеренно. Раньше ждали — и экран оставался тёмным лишние
# двадцать секунд, пока поднимается NetworkManager. Но часам сеть не нужна:
# время идёт само, датчики на шине, а погода и брокер подключаются позже и
# переподключаются сами, это уже проверено обрывом брокера.
#
# Побочная польза важнее скорости: при сломанном WiFi блок всё равно
# покажет часы и воздух, а не останется чёрным.
