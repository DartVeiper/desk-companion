...
#
#   bash backup-repo.sh                       # на плату, адрес из .deploy-target
...
# а удалённый репозиторий — от «удалил ветку и потом силой затёр историю».
...
# Адрес берём там же, где deploy.sh, а не зашитый: зашитый — это чужая
# домашняя сеть в чужом репозитории, то есть мусор для всех, кроме автора.
REMEMBERED="$HERE/.deploy-target"
if [ -n "${1:-}" ]; then
    TARGET="$1"
